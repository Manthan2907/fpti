...
class FinanceDashboard:
    def create_bank_accounts_tab(self):
        self.bank_accounts_frame = self.create_glassmorphism_frame(self.notebook, self.colors["bg_primary"])
        self.notebook.add(self.bank_accounts_frame, text="🏦 BANK ACCOUNTS")
        title = tk.Label(self.bank_accounts_frame, text="🏦 BANK ACCOUNTS MANAGER", font=("Segoe UI", 20, "bold"), fg=self.colors["accent_cyan"], bg=self.colors["bg_primary"])
        title.pack(pady=20)

        # --- Create Account Section ---
        create_frame = self.create_glassmorphism_frame(self.bank_accounts_frame)
        create_frame.pack(pady=10, padx=20, fill="x")
        create_container = tk.Frame(create_frame, bg=self.colors["bg_card"])
        create_container.pack(pady=10)
        tk.Label(create_container, text="Bank Name:", bg=self.colors["bg_card"], fg=self.colors["text_primary"]).grid(row=0, column=0, padx=5, pady=5)
        bank_name_var = tk.StringVar()
        tk.Entry(create_container, textvariable=bank_name_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=1, padx=5, pady=5)
        tk.Label(create_container, text="Initial Deposit:", bg=self.colors["bg_card"], fg=self.colors["text_primary"]).grid(row=0, column=2, padx=5, pady=5)
        bank_balance_var = tk.DoubleVar(value=0.0)
        tk.Entry(create_container, textvariable=bank_balance_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=3, padx=5, pady=5)
        def add_account():
            name = bank_name_var.get().strip()
            bal = bank_balance_var.get()
            if not name:
                messagebox.showwarning("Missing Name", "Please enter a bank name.")
                return
            if bal > self.cash_balance:
                messagebox.showwarning("Insufficient Cash", "Not enough cash for initial deposit.")
                return
            if not hasattr(self, "bank_accounts"):
                self.bank_accounts = []
            self.bank_accounts.append({"name": name, "balance": bal, "fds": [], "loans": []})
            self.cash_balance -= bal
            self.save_data()
            self.update_bank_accounts_display()
            self.update_dashboard()
        tk.Button(create_container, text="➕ CREATE ACCOUNT", command=add_account, bg=self.colors["accent_cyan"], fg=self.colors["bg_primary"], font=("Segoe UI", 11, "bold"), padx=15, pady=8, relief="flat").grid(row=0, column=4, padx=5, pady=5)

        # --- Open FD Section ---
        fd_frame = self.create_glassmorphism_frame(self.bank_accounts_frame)
        fd_frame.pack(pady=10, padx=20, fill="x")
        fd_container = tk.Frame(fd_frame, bg=self.colors["bg_card"])
        fd_container.pack(pady=10)
        tk.Label(fd_container, text="Open FD for Selected Account:", bg=self.colors["bg_card"], fg=self.colors["accent_cyan"], font=("Segoe UI", 12, "bold")).grid(row=0, column=0, padx=5, pady=5)
        fd_amount_var = tk.DoubleVar(value=1000)
        tk.Entry(fd_container, textvariable=fd_amount_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=1, padx=5, pady=5)
        tk.Label(fd_container, text="Term (min):", bg=self.colors["bg_card"], fg=self.colors["text_primary"]).grid(row=0, column=2, padx=5, pady=5)
        fd_term_var = tk.IntVar(value=60)
        tk.Entry(fd_container, textvariable=fd_term_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=3, padx=5, pady=5)
        tk.Label(fd_container, text="Interest/min (%):", bg=self.colors["bg_card"], fg=self.colors["text_primary"]).grid(row=0, column=4, padx=5, pady=5)
        fd_rate_var = tk.DoubleVar(value=0.1)
        tk.Entry(fd_container, textvariable=fd_rate_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=5, padx=5, pady=5)
        def open_fd():
            idx = self.bank_accounts_listbox.curselection()
            if not idx:
                messagebox.showwarning("Select Account", "Select a bank account to open FD.")
                return
            amt = fd_amount_var.get()
            if amt > self.cash_balance:
                messagebox.showwarning("Insufficient Cash", "Not enough cash for FD.")
                return
            term = fd_term_var.get()
            rate = fd_rate_var.get()
            acc = self.bank_accounts[idx[0]]
            acc["fds"].append({"amount": amt, "term": term, "rate": rate, "start": datetime.now().isoformat()})
            self.cash_balance -= amt
            self.save_data()
            self.update_bank_accounts_display()
            self.update_dashboard()
        tk.Button(fd_container, text="Open FD", command=open_fd, bg=self.colors["accent_cyan"], fg=self.colors["bg_primary"], font=("Segoe UI", 11, "bold"), padx=15, pady=8, relief="flat").grid(row=0, column=6, padx=5, pady=5)

        # --- Take Loan Section ---
        loan_frame = self.create_glassmorphism_frame(self.bank_accounts_frame)
        loan_frame.pack(pady=10, padx=20, fill="x")
        loan_container = tk.Frame(loan_frame, bg=self.colors["bg_card"])
        loan_container.pack(pady=10)
        tk.Label(loan_container, text="Take Loan from Selected Account:", bg=self.colors["bg_card"], fg=self.colors["accent_cyan"], font=("Segoe UI", 12, "bold")).grid(row=0, column=0, padx=5, pady=5)
        loan_amt_var = tk.DoubleVar(value=1000)
        tk.Entry(loan_container, textvariable=loan_amt_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=1, padx=5, pady=5)
        tk.Label(loan_container, text="Term (min):", bg=self.colors["bg_card"], fg=self.colors["text_primary"]).grid(row=0, column=2, padx=5, pady=5)
        loan_term_var = tk.IntVar(value=60)
        tk.Entry(loan_container, textvariable=loan_term_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=3, padx=5, pady=5)
        tk.Label(loan_container, text="Interest/min ($):", bg=self.colors["bg_card"], fg=self.colors["text_primary"]).grid(row=0, column=4, padx=5, pady=5)
        loan_rate_var = tk.DoubleVar(value=2.0)
        tk.Entry(loan_container, textvariable=loan_rate_var, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]).grid(row=0, column=5, padx=5, pady=5)
        def take_loan():
            idx = self.bank_accounts_listbox.curselection()
            if not idx:
                messagebox.showwarning("Select Account", "Select a bank account to take loan from.")
                return
            amt = loan_amt_var.get()
            term = loan_term_var.get()
            rate = loan_rate_var.get()
            acc = self.bank_accounts[idx[0]]
            acc["loans"].append({"amount": amt, "term": term, "rate": rate, "start": datetime.now().isoformat(), "remaining": amt})
            self.cash_balance += amt
            self.save_data()
            self.update_bank_accounts_display()
            self.update_dashboard()
        tk.Button(loan_container, text="Take Loan", command=take_loan, bg=self.colors["accent_cyan"], fg=self.colors["bg_primary"], font=("Segoe UI", 11, "bold"), padx=15, pady=8, relief="flat").grid(row=0, column=6, padx=5, pady=5)

        # --- List of bank accounts ---
        list_frame = tk.Frame(self.bank_accounts_frame, bg=self.colors["bg_card"])
        list_frame.pack(fill="both", expand=True, padx=20, pady=10)
        self.bank_accounts_listbox = tk.Listbox(list_frame, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"], height=10, selectbackground=self.colors["accent_purple"], font=("Consolas", 11))
        self.bank_accounts_listbox.pack(side="left", fill="both", expand=True, padx=(0, 8))
        scrollbar = tk.Scrollbar(list_frame, orient="vertical", command=self.bank_accounts_listbox.yview)
        scrollbar.pack(side="right", fill="y")
        self.bank_accounts_listbox.config(yscrollcommand=scrollbar.set)
        self.update_bank_accounts_display()

    def update_bank_accounts_display(self):
        self.bank_accounts_listbox.delete(0, tk.END)
        if not hasattr(self, "bank_accounts") or not self.bank_accounts:
            self.bank_accounts_listbox.insert(tk.END, "No bank accounts created.")
        else:
            for acc in self.bank_accounts:
                self.bank_accounts_listbox.insert(
                    tk.END,
                    f"{acc['name']} | Balance: ${acc['balance']:,.2f} | FDs: {len(acc['fds'])} | Loans: {len(acc['loans'])}",
                )

    # --- Plaid API Integration ---
    def create_plaid_tab(self):
        # Plaid is a real-world API for syncing with real bank accounts. This is a placeholder tab.
        # If you do not use Plaid, you can remove this tab or leave it as a future integration point.
        self.plaid_frame = self.create_glassmorphism_frame(self.notebook, self.colors["bg_primary"])
        self.notebook.add(self.plaid_frame, text="🏦 BANK SYNC")
        title = tk.Label(self.plaid_frame, text="🏦 LINK BANK ACCOUNTS (Plaid)", font=("Segoe UI", 20, "bold"), fg=self.colors["accent_cyan"], bg=self.colors["bg_primary"])
        title.pack(pady=20)
        tk.Label(self.plaid_frame, text="Plaid integration is not active in this app. This is a placeholder for future real bank sync.", font=("Segoe UI", 12), fg=self.colors["text_secondary"], bg=self.colors["bg_primary"]).pack(pady=10)

    def create_credit_cards_tab(self):
        """Credit Card management tab"""
        self.credit_cards_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        self.notebook.add(self.credit_cards_frame, text="💳 CREDIT CARDS")
        title = tk.Label(
            self.credit_cards_frame,
        ...
        title.pack(pady=20)
        # Controls
        controls_frame = self.create_glassmorphism_frame(self.credit_cards_frame)
        ...
        tk.Button(
            controls_container,
            text="❌ REMOVE CARD",
            command=self.remove_credit_card,
        ...
        # List of cards
        list_frame = tk.Frame(self.credit_cards_frame, bg=self.colors["bg_card"])
        list_frame.pack(fill="both", expand=True, padx=20, pady=10)
        ...
        self.cards_listbox.pack(side="left", fill="both", expand=True, padx=(0, 8))
        scrollbar = tk.Scrollbar(
            list_frame, orient="vertical", command=self.cards_listbox.yview
        )
        scrollbar.pack(side="right", fill="y")
        self.cards_listbox.config(yscrollcommand=scrollbar.set)
        self.update_credit_cards_display()

    def create_real_estate_tab(self):
        self.real_estate_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        self.notebook.add(self.real_estate_frame, text="🏠 REAL ESTATE")
        title = tk.Label(
            self.real_estate_frame,
        ...
        tk.Button(
            self.real_estate_frame,
            text="➕ Add Property",
            command=self.add_real_estate_property,
            bg=self.colors["accent_cyan"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        self.real_estate_listbox = tk.Listbox(
            self.real_estate_frame,
            bg=self.colors["bg_secondary"],
            fg=self.colors["text_primary"],
            height=10,
        ...
        self.real_estate_listbox.pack(fill="both", expand=True, padx=20, pady=10)
        self.update_real_estate_display()
        tk.Button(
            self.real_estate_frame,
            text="✏️ Edit Property",
            command=self.edit_real_estate_property,
        ...
        tk.Button(
            ...
            command=self.remove_real_estate_property,
            bg=self.colors["danger"],
            fg=self.colors["text_primary"],
            font=("Segoe UI", 10, "bold"),
            padx=10,
    ...
    def add_real_estate_property(self):
        dialog = tk.Toplevel(self.root)
        dialog.title("Add Real Estate Property")
        ...
        value_var = tk.DoubleVar(value=100000)
        tk.Entry(
            dialog,
        ...
        def add():
            ...
            if self.cash_balance < price:
                messagebox.showwarning(
                    "Insufficient Funds",
                    "You do not have enough cash to buy this property.",
                )
                return
            prop = {
                "name": name_var.get(),
            ...
            if not hasattr(self, "real_estate_assets"):
                self.real_estate_assets = []
            self.real_estate_assets.append(prop)
    ...
    def update_real_estate_display(self):
        self.real_estate_listbox.delete(0, tk.END)
        if not hasattr(self, "real_estate_assets") or not self.real_estate_assets:
            self.real_estate_listbox.insert(tk.END, "No properties added.")
        else:
            for prop in self.real_estate_assets:
                self.real_estate_listbox.insert(
                    tk.END,
                    f"{prop['name']} | Value: ${prop['value']:,.2f} | Location: {prop['location']}",
                )

    def edit_real_estate_property(self):
        idx = self.real_estate_listbox.curselection()
        if (
            not idx
            or not hasattr(self, "real_estate_assets")
            or idx[0] >= len(self.real_estate_assets)
        ):
            return
        prop = self.real_estate_assets[idx[0]]
        dialog = tk.Toplevel(self.root)
        dialog.title("Edit Real Estate Property")
        dialog.geometry("400x350")
        dialog.configure(bg=self.colors["bg_primary"])
        tk.Label(
            dialog,
        ...
        name_var = tk.StringVar(value=prop["name"])
        tk.Entry(
            dialog,
            textvariable=name_var,
            bg=self.colors["bg_secondary"],
        ...
        value_var = tk.DoubleVar(value=prop["value"])
        tk.Entry(
            dialog,
            textvariable=value_var,
            bg=self.colors["bg_secondary"],
        ...
        location_var = tk.StringVar(value=prop["location"])
        tk.Entry(
            dialog,
            textvariable=location_var,
            bg=self.colors["bg_secondary"],
        ...
        def save():
            prop["name"] = name_var.get()
            prop["value"] = value_var.get()
            prop["location"] = location_var.get()
            self.save_data()
            self.update_real_estate_display()
            self.update_dashboard()
            dialog.destroy()
    ...
    def remove_real_estate_property(self):
        idx = self.real_estate_listbox.curselection()
        if (
            not idx
            or not hasattr(self, "real_estate_assets")
            ...
            return
        del self.real_estate_assets[idx[0]]
        self.save_data()
        self.update_real_estate_display()
        self.update_dashboard()
    ...
    def update_credit_cards_display(self):
        self.cards_listbox.delete(0, tk.END)
        if not hasattr(self, "credit_cards") or not self.credit_cards:
            self.cards_listbox.insert(tk.END, "No credit cards added.")
        else:
            for card in self.credit_cards:
                self.cards_listbox.insert(
                    tk.END,
                    f"{card['name']} | Balance: ${card['balance']:.2f} | Limit: ${card['limit']:.2f} | Interest: {card['interest_rate']}%",
                )
    ...
    def add_credit_card(self):
        ...
        limit_var = tk.DoubleVar(value=1000)
        tk.Entry(
            dialog,
        ...
        tk.Label(
            dialog,
            text="Interest Rate (% per min):",
            bg=self.colors["bg_primary"],
            fg=self.colors["text_primary"],
        ).pack(pady=5)
        interest_var = tk.DoubleVar(value=2.0)
        tk.Entry(
            dialog,
        ...
        def add():
            card = {
                ...
                "limit": limit_var.get(),
                "balance": 0.0,
                "interest_rate": interest_var.get(),
                "last_interest_time": datetime.now().isoformat(),
            }
            if not hasattr(self, "credit_cards"):
                self.credit_cards = []
    ...
    def pay_credit_card(self):
        idx = self.cards_listbox.curselection()
        if (
            not idx
            or not hasattr(self, "credit_cards")
            or idx[0] >= len(self.credit_cards)
        ):
            return
        card = self.credit_cards[idx[0]]
        dialog = tk.Toplevel(self.root)
        dialog.title("Pay Credit Card")
        dialog.geometry("350x200")
        dialog.configure(bg=self.colors["bg_primary"])
        tk.Label(
            dialog,
            text=f"Pay {card['name']} (Balance: ${card['balance']:.2f})",
            bg=self.colors["bg_primary"],
            fg=self.colors["text_primary"],
        ).pack(pady=10)
        amount_var = tk.DoubleVar(value=card["balance"])
        tk.Entry(
            dialog,
        ...
        def pay():
            amt = min(amount_var.get(), card["balance"], self.cash_balance)
            card["balance"] -= amt
            self.cash_balance -= amt
            self.save_data()
            self.update_credit_cards_display()
    ...
    def remove_credit_card(self):
        ...
        if (
            ...
            return
        del self.credit_cards[idx[0]]
        self.save_data()
        self.update_credit_cards_display()
        self.update_dashboard()

    def use_credit_card_for_goal(self, amount):
        # Use first card with available limit
        if not hasattr(self, "credit_cards") or not self.credit_cards:
            return False
        for card in self.credit_cards:
    ...
    def accrue_credit_card_interest(self):
        if not hasattr(self, "credit_cards") or not self.credit_cards:
            return
        now = datetime.now()
        for card in self.credit_cards:
            try:
                last = datetime.fromisoformat(
                    card.get("last_interest_time", now.isoformat())
                )
                mins = int((now - last).total_seconds() // 60)
                if mins > 0 and card["balance"] > 0:
                    rate = card["interest_rate"] / 100.0
                    for _ in range(mins):
                        card["balance"] += card["balance"] * rate
                    card["last_interest_time"] = now.isoformat()
            except Exception:
                card["last_interest_time"] = now.isoformat()
        self.save_data()
        self.update_credit_cards_display()
    ...
    def categorize_transaction(self, description, amount):
        """Rule-based categorization for a transaction."""
        ...
        if amount > 0:
            ...
            if "rent" in desc:
                return "Expense: Rent"
            if "grocery" in desc or "supermarket" in desc or "food" in desc:
                return "Expense: Groceries"
            if "restaurant" in desc or "cafe" in desc:
                return "Expense: Dining"
            if "uber" in desc or "taxi" in desc or "ola" in desc or "cab" in desc:
                return "Expense: Transport"
            if "electric" in desc or "water" in desc or "utility" in desc:
            ...
            if "travel" in desc or "flight" in desc or "hotel" in desc:
                return "Expense: Travel"
            if "shopping" in desc or "amazon" in desc or "flipkart" in desc:
                return "Expense: Shopping"
            if "fuel" in desc or "petrol" in desc or "gas" in desc:
                return "Expense: Fuel"
            if "school" in desc or "tuition" in desc or "college" in desc:
                return "Expense: Education"
            if "phone" in desc or "mobile" in desc:
                return "Expense: Phone"
            if "subscription" in desc or "netflix" in desc or "prime" in desc:
    ...
    def setup_styles(self):
        ...
        style.configure(
            ...
            foreground=self.colors["accent_cyan"],  # Higher contrast
            padding=[50, 18],  # Slightly increased padding
            borderwidth=2,
            relief="flat",
            font=("Segoe UI", 16, "bold"),  # Moderately larger font for visibility
        )
    ...
    def load_data(self):
        ...
        try:
            if os.path.exists(self.data_file):
                ...
                if data is None:
                    ...
                    self.portfolio = data.get(
                        "portfolio", {"CASH": {"shares": 0.0, "avg_price": 1.0}}
                    )
                ...
                if tx is not None:
                    ...
                    self.transactions_df = None
                # Always load these from file if present
                ...
                self.real_estate_assets = data.get("real_estate_assets", [])
                self.credit_cards = data.get("credit_cards", [])
                self.bank_accounts = data.get("bank_accounts", [])
                # Load last interest time for calculating offline interest
                self.last_interest_time = data.get(
                    "last_interest_time", datetime.now().isoformat()
                )
                ...
                # Ensure credit_cards and bank_accounts always exist
                if not hasattr(self, "credit_cards"):
                    self.credit_cards = []
                if not hasattr(self, "bank_accounts"):
                    self.bank_accounts = []
            ...
            messagebox.showerror(
                "Data Loading Error", "Could not load previous data. Starting fresh."
            )
            self.csv_loaded = False
    ...
    def save_data(self):
        ...
        try:
            ...
            data = {
                ...
                "last_interest_time": getattr(
                    self, "last_interest_time", datetime.now().isoformat()
                ),
                "real_estate_assets": getattr(self, "real_estate_assets", []),
                "credit_cards": getattr(self, "credit_cards", []),
            ...
            if getattr(self, "data_corrupted", False):
                ...
                if resp:
                    ...
                    messagebox.showinfo(
                        "Save",
                        f"Data saved and original file overwritten ({self.data_file}).",
                    )
                else:
                    ...
                    messagebox.showinfo(
                        "Save",
                        f"Data saved to recovered file: {recovered}. Original corrupted file preserved.",
                    )
            else:
            ...
            messagebox.showerror("Data Saving Error", f"Failed to save data: {str(e)}")
    ...
    def setup_ui(self):
        ...
        self.dashboard_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        ...
        # Add bank accounts tab (new)
        self.create_bank_accounts_tab()

        # Add credit cards tab
        self.create_credit_cards_tab()

        # Add Plaid API tab
        self.create_plaid_tab()
        ...
        canvas = tk.Canvas(
            self.dashboard_frame, bg=self.colors["bg_primary"], highlightthickness=0
        )
        scrollbar = tk.Scrollbar(
            self.dashboard_frame, orient="vertical", command=canvas.yview
        )
        ...
        tk.Label(
            scrollable_frame,
            text="⚡ UNIFIED FINANCIAL DASHBOARD ⚡",
        ...
        tk.Button(
            loan_card,
            text="➕ TAKE LOAN",
            command=self.manage_loans_dialog,
            bg=self.colors["accent_cyan"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 10, "bold"),
            padx=10,
        ...
        self.portfolio_summary_frame = tk.Frame(
            portfolio_section, bg=self.colors["bg_card"]
        )
    ...
    def update_dashboard(self):
        ...
        try:
            ...
            self.portfolio_value_label.config(text=f"${port_val:,.2f}")
            # Update net worth (include credit cards)
            net_worth = self.calculate_net_worth()
            self.networth_label.config(text=f"${net_worth:,.2f}")
            # Update loans
            if self.loans:
                self.loans_label.config(
                    text=f"{len(self.loans)} active | ${sum(l.get('remaining', l.get('amount', 0)) for l in self.loans):,.2f}"
                )
            else:
                self.loans_label.config(text="No active loans")
            # Update credit cards
            if hasattr(self, "credit_cards") and self.credit_cards:
                total_cc = sum(card["balance"] for card in self.credit_cards)
                if hasattr(self, "cards_listbox"):
                    self.cards_listbox.delete(0, tk.END)
                    for card in self.credit_cards:
                        self.cards_listbox.insert(
                            tk.END,
    ...
    def calculate_net_worth(self):
        ...
        for loan in self.loans:
            net_worth -= loan.get("remaining", loan.get("amount", 0))
        # Subtract credit card balances
        if hasattr(self, "credit_cards"):
            for card in self.credit_cards:
                net_worth -= card.get("balance", 0)
        # Add real estate asset values
        if hasattr(self, "real_estate_assets"):
            for prop in self.real_estate_assets:
                net_worth += prop.get("value", 0)
        return net_worth
    ...
    def start_cash_interest_scheduler(self):
        ...
        try:
            ...
            # Also accrue credit card interest
            self.accrue_credit_card_interest()
    ...
    def update_recent_transactions(self):
        ...
        if (
            not hasattr(self, "transactions_df")
            or self.transactions_df is None
            or self.transactions_df.empty
        ):
    ...
    def process_loans_minute(self):
        ...
        for loan in list(self.loans):
            try:
                ...
                loan["remaining"] = max(
                    0.0, loan.get("remaining", loan.get("amount", 0)) - interest
                )
                ...
                self.notify_ui(
                    f"Loan interest charged: ${interest:,.2f}. Minutes left: {loan['mins_left']}"
                )
    ...
    def manage_loans_dialog(self):
        ...
        scrollbar = tk.Scrollbar(
            list_frame, orient="vertical", command=loan_listbox.yview
        )
        scrollbar.pack(side="right", fill="y")
        ...
        for idx, loan in enumerate(self.loans):
            ...
            loan_listbox.insert(
                "end",
                f"{idx+1}. ${amt:,.2f} | remaining: ${rem:,.2f} | ${ipm:.2f}/min | {mins} mins left | id:{pid}",
            )
        ...
        # --- Take Loan Section ---
        take_loan_frame = tk.Frame(control_frame, bg=self.colors["bg_primary"])
        take_loan_frame.pack(side="top", fill="x", pady=(0, 10))
        tk.Label(
            take_loan_frame,
            text="Take Loan:",
            font=("Segoe UI", 11, "bold"),
        ...
        loan_interest_var = tk.DoubleVar(value=2.0)
        tk.Entry(
            take_loan_frame,
            textvariable=loan_interest_var,
            bg=self.colors["bg_secondary"],
            fg=self.colors["text_primary"],
            width=8,
        ...
        def add_loan():
            amt = loan_amt_var.get()
            mins = loan_term_var.get()
            ipm = loan_interest_var.get()
            if amt <= 0 or mins <= 0 or ipm < 0:
                messagebox.showwarning(
                    "Invalid Input", "Enter valid loan amount, duration, and interest."
                )
                return
            loan = {
                "id": int(time.time()),
            ...
            self.update_dashboard()
            dialog.destroy()
            self.notify_ui(f"Loan of ${amt:,.2f} taken for {mins} mins.")
        ...
        tk.Button(
            ...
            padx=10,
            pady=4,
        ).grid(row=1, column=6, padx=8)

        # --- Repay Section ---
        # --- Repay Section ---
        repay_frame = tk.Frame(control_frame, bg=self.colors["bg_primary"])
        repay_frame.pack(side="top", fill="x", pady=(0, 10))
        tk.Label(
            repay_frame,
            text="Repay amount ($):",
            font=("Segoe UI", 11),
        ...
        repay_entry = tk.Entry(
            repay_frame,
            textvariable=repay_var,
            bg=self.colors["bg_secondary"],
            fg=self.colors["text_primary"],
            width=18,
        )
        ...
        def repay_selected():
            ...
            if loan["remaining"] <= 0.0:
                ...
                self.notify_ui(
                    f"Repayment made: ${repay_amt:,.2f}. Remaining: ${loan['remaining']:,.2f}"
                )
            ...
            for j, ln in enumerate(self.loans):
                ...
                loan_listbox.insert(
                    "end",
                    f"{j+1}. ${amt:,.2f} | remaining: ${rem:,.2f} | ${ipm:.2f}/min | {mins} mins left | id:{pid}",
                )

        tk.Button(
            repay_frame,
            text="Repay Selected",
            command=repay_selected,
            bg=self.colors["accent_cyan"],
            fg=self.colors["bg_primary"],
            padx=12,
            pady=6,
    ...
    def update_portfolio_summary(self):
        ...
        sorted_holdings = sorted(
            ...
            * x[1].get("current_price", x[1].get("avg_price", 0)),
            reverse=True,
        )
        ...
        for symbol, data in top_holdings:
            holding_frame = tk.Frame(
                self.portfolio_summary_frame, bg=self.colors["bg_card"]
            )
            ...
            gain_loss_pct = (
                (gain_loss / (shares * avg_price) * 100)
                if (shares * avg_price) > 0
                else 0
            )
            ...
            tk.Label(
                holding_frame,
                text=f"{symbol}: ${current_value:,.0f}",
                font=("Segoe UI", 10, "bold"),
                fg=self.colors["accent_pink"],
                bg=self.colors["bg_card"],
            ).pack(side="left", padx=5)
            ...
            gain_color = (
                self.colors["success"] if gain_loss >= 0 else self.colors["danger"]
            )
            tk.Label(
                holding_frame,
                text=f"({gain_loss_pct:+.1f}%)",
                font=("Segoe UI", 10),
                fg=gain_color,
                bg=self.colors["bg_card"],
    ...
    def update_goals_summary(self):
        ...
        if not self.financial_goals:
            tk.Label(
                self.goals_summary_frame,
                text="No financial goals set",
                font=("Segoe UI", 10),
                fg=self.colors["text_secondary"],
                bg=self.colors["bg_card"],
            ).pack()
        ...
        for goal in top_goals:
            ...
            tk.Label(
                goal_frame,
                text=f"${current:,.0f}/${target:,.0f} ({progress:.0f}%)",
                font=("Segoe UI", 10),
                fg=progress_color,
                bg=self.colors["bg_card"],
        ...
        tk.Label(
            total_frame,
            text=f"Overall: {overall_progress:.0f}% (${total_current:,.0f}/${total_target:,.0f})",
            font=("Segoe UI", 11, "bold"),
            fg=overall_color,
            bg=self.colors["bg_card"],
        ).pack(side="left", padx=5)
    ...
    def create_expenses_tab(self):
        ...
        self.main_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        ...
        self.manual_btn = tk.Button(
            ...
            command=self.add_manual_transaction,
        ...
        self.allocate_btn = tk.Button(
            btn_container,
            text="💰 ALLOCATE FUNDS",
            command=self.allocate_funds_to_portfolio,
            bg=self.colors["warning"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 12, "bold"),
            padx=25,
        ...
        scrollbar = tk.Scrollbar(
            text_container, orient="vertical", command=self.summary_text.yview
        )
...
{self.colors['success']}Ready to analyze your financial data! 🚀{self.colors['text_primary']}
    ...
    def create_portfolio_tab(self):
        ...
        self.portfolio_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        ...
        tk.Button(
            controls_container,
            text="💰 SELL INVESTMENT",
            command=self.sell_investment,
            bg=self.colors["success"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        tk.Button(
            ...
            command=self.remove_investment,
            bg=self.colors["danger"],
            fg=self.colors["text_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        tk.Button(
            controls_container,
            text="🔄 UPDATE PRICES",
            command=self.update_portfolio_prices,
            bg=self.colors["accent_purple"],
            fg=self.colors["text_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        holdings_scroll = tk.Scrollbar(
            holdings_frame, orient="vertical", command=self.holdings_text.yview
        )
    ...
    def create_stock_tab(self):
        ...
        self.stock_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        ...
        stock_scrollbar = tk.Scrollbar(
            text_container, orient="vertical", command=self.stock_text.yview
        )
...
TIP: Enter any stock symbol above and click 'Fetch Data' for real-time information!
    ...
    def create_stock_chart_ui(self, hist, symbol):
        ...
        try:
            ...
            change_text = (
                f"Last: ${current_price:.2f} ({change:+.2f}, {change_pct:+.1f}%)"
            )
    ...
    def create_goals_tab(self):
        ...
        self.goals_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        ...
        tk.Button(
            controls_container,
            text="➕ ADD GOAL",
            command=self.add_financial_goal,
            bg=self.colors["accent_cyan"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        tk.Button(
            controls_container,
            text="✏️ UPDATE GOAL",
            command=self.update_financial_goal,
            bg=self.colors["accent_purple"],
            fg=self.colors["text_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        tk.Button(
            controls_container,
            text="❌ DELETE GOAL",
            command=self.delete_financial_goal,
            bg=self.colors["danger"],
            fg=self.colors["text_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        tk.Button(
            controls_container,
            text="💰 ALLOCATE FUNDS",
            command=self.allocate_funds_to_goal,
            bg=self.colors["warning"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
        ...
        tk.Button(
            controls_container,
            text="🔄 REFRESH",
            command=self.update_goals_display,
            bg=self.colors["success"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 11, "bold"),
            padx=15,
    ...
    def create_currency_tab(self):
        ...
        self.currency_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
        ...
        self.currency_result = tk.Label(
            self.currency_frame,
            text="Enter amount and convert",
            font=("Segoe UI", 14, "bold"),
            fg=self.colors["warning"],
            bg=self.colors["bg_primary"],
        )
    ...
    def create_charts_tab(self):
        ...
        self.charts_frame = self.create_glassmorphism_frame(
            self.notebook, self.colors["bg_primary"]
        )
    ...
    def upload_csv(self):
        """Import transactions from a CSV file, auto-categorize if needed."""
        file_path = filedialog.askopenfilename(filetypes=[("CSV Files", "*.csv")])
        if not file_path:
            return
        try:
            df = pd.read_csv(file_path)
            # Accept CSVs with at least date, amount; description/category optional
            required = {"date", "amount"}
            missing = required - set(df.columns)
            if missing:
                messagebox.showerror(
                    "CSV Import Error", f"Missing columns: {', '.join(missing)}"
                )
                return
            if "description" not in df.columns:
                df["description"] = (
                    df.get("category", "") if "category" in df.columns else ""
                )
            # Auto-categorize
            df["category"] = df.apply(
                lambda row: self.categorize_transaction(
                    row.get("description", ""), row["amount"]
                ),
                axis=1,
            )
            ...
            if self.transactions_df is None:
                ...
                self.transactions_df = pd.concat(
                    [self.transactions_df, df], ignore_index=True
                )
            ...
            self.update_recent_transactions()
            self.update_dashboard()
            self.notify_ui("CSV imported and categorized!")
        except Exception as e:
...
"""
    ...
    def add_investment(self):
        ...
        payment_options = ["Cash", "Credit Card", "Bank Account"]
        ...
        payment_menu.grid(row=4, column=1, pady=5, padx=10)
        ...
        def validate_symbol():
            ...
            if company_name:
                ...
                company_label.config(
                    text=f"Invalid symbol: {error}", fg=self.colors["danger"]
                )
        ...
        def fetch_current_price():
            ...
            if current_price:
                ...
                status_label.config(
                    text=f"Price fetched: ${current_price:.2f}",
                    fg=self.colors["success"],
                )
            else:
            ...
            try:
                ...
                if payment_method == "Cash":
                    if total_purchase_cost > self.cash_balance:
                        messagebox.showerror(
                            "Error",
                            f"Insufficient cash balance. Required: ${total_purchase_cost:,.2f}, Available: ${self.cash_balance:,.2f}",
                        )
                        return
                    self.cash_balance -= total_purchase_cost
                    pay_desc = "Paid with Cash"
                elif payment_method == "Credit Card":
                    if not hasattr(self, "credit_cards") or not self.credit_cards:
                        messagebox.showerror("Error", "No credit cards available.")
                        return
                    # Use first card with available limit
                    for card in self.credit_cards:
                        available = card["limit"] - card["balance"]
                        if total_purchase_cost <= available:
                            card["balance"] += total_purchase_cost
                            pay_desc = f"Paid with Credit Card: {card['name']}"
                            break
                    else:
                        messagebox.showerror(
                            "Error", "No credit card has enough available limit."
                        )
                        return
                elif payment_method == "Bank Account":
                    # Placeholder: always allow for now
                    pay_desc = "Paid with Bank Account"
                else:
                ...
                if symbol in self.portfolio:
                    ...
                    total_cost = (existing_shares * existing_avg_price) + (
                ...
                self.add_transaction_record(
                    ...
                    f"Investment: {symbol}",
                    -total_purchase_cost,
                    f"Purchased {shares} shares at ${current_price:,.2f} | {pay_desc}",
                )
                ...
                messagebox.showinfo(
                    "Success",
                    f"Added {symbol} to portfolio!\nPurchase cost: ${total_purchase_cost:,.2f}",
                )
            except ValueError:
                messagebox.showerror(
                    "Error", "Please enter valid numbers for shares and price"
                )
    ...
    def add_manual_transaction(self):
        """Add a manual transaction to the transactions dataframe, auto-categorize."""
        dialog = tk.Toplevel(self.root)
        ...
        description_entry.grid(row=2, column=1, pady=5, padx=10)
        ...
        def save_transaction():
            try:
                ...
                if self.transactions_df is not None and not self.transactions_df.empty:
                    self.transactions_df = pd.concat(
                        [self.transactions_df, new_transaction], ignore_index=True
                    )
                else:
                ...
                dialog.destroy()
                messagebox.showinfo(
                    "Success", f"Added transaction: {category} ${amount:,.2f}"
                )
    ...
    def sell_investment(self):
        ...
        symbol_combo = ttk.Combobox(
            selection_frame,
            textvariable=symbol_var,
            values=list(self.portfolio.keys()),
            font=("Segoe UI", 11),
            width=20,
        )
        ...
        shares_entry = tk.Entry(
            sell_frame,
            textvariable=shares_to_sell_var,
            font=("Segoe UI", 11),
            bg=self.colors["bg_secondary"],
            fg=self.colors["text_primary"],
            insertbackground=self.colors["accent_cyan"],
        ...
        def update_investment_details(event=None):
            ...
            if symbol and symbol in self.portfolio:
                ...
                value_label.config(
                    text=f"Current Value: ${value:,.2f} (${current_price:,.2f}/share)"
                )
        ...
        def sell_shares():
            ...
            try:
                ...
                if shares_to_sell <= 0:
                    messagebox.showerror(
                        "Error", "Please enter a valid number of shares to sell"
                    )
                    return
                ...
                if shares_to_sell > available_shares:
                    messagebox.showerror(
                        "Error",
                        f"Cannot sell {shares_to_sell} shares. Only {available_shares} available.",
                    )
                    return
                ...
                messagebox.showinfo(
                    "Success",
                    f"Sold {shares_to_sell} shares of {symbol} for ${proceeds:,.2f}!\nCash balance: ${self.cash_balance:,.2f}",
                )
    ...
    def upload_csv(self):
        ...
        try:
            ...
            self.status_label.config(
                text=f"✅ {action.capitalize()}: {filename} ({len(new_transactions_df)} transactions)",
                fg=self.colors["success"],
            )
            ...
            messagebox.showinfo(
                "Success",
                f"✨ {action.capitalize()} {len(new_transactions_df)} transactions successfully!",
            )
    ...
    def allocate_funds_to_portfolio(self):
        ...
        tk.Label(
            dialog,
            text="Allocate Funds to Portfolio",
            font=("Segoe UI", 16, "bold"),
            fg=self.colors["accent_cyan"],
            bg=self.colors["bg_primary"],
        ).pack(pady=12)
        ...
        def allocate_action():
            ...
            self.add_transaction_record(
                now, "Allocate: Portfolio", -amt, f"Allocated ${amt:,.2f} ({mode})"
            )

            if mode == "cash":
                # Add/Update CASH holding (shares = dollars)
                if "CASH" in self.portfolio:
                    self.portfolio["CASH"]["shares"] = (
                        self.portfolio["CASH"].get("shares", 0) + amt
                    )
                ...
                if not sym:
                    messagebox.showerror(
                        "Error", "Please enter a symbol for single buy"
                    )
                    return
                price, err = self.get_current_stock_price(sym)
                if price is None:
                    messagebox.showerror(
                        "Error", f"Could not fetch price for {sym}: {err}"
                    )
                    return
                ...
                if sym in self.portfolio:
                    ...
                    tot_cost = (
                        ex["shares"]
                        * ex.get("avg_price", ex.get("current_price", price))
                        + shares * price
                    )
                ...
                if not targets:
                    ...
                    for s in targets:
                        ...
                        if price is None:
                            price = self.portfolio[s].get(
                                "current_price", self.portfolio[s].get("avg_price", 0)
                            )
                        ...
                        self.add_transaction_record(
                            ...
                            f"Investment: {s}",
                            -per,
                            f"Allocated ${per:,.2f} to {s} (approx {shares:.6f} shares)",
                        )
            ...
            dialog.destroy()
            messagebox.showinfo(
                "Success", f"Allocated ${amt:,.2f} to portfolio ({mode})"
            )
    ...
    def remove_investment(self):
        ...
        tk.Label(
            dialog,
            text="Select Investment to Remove",
            font=("Segoe UI", 16, "bold"),
            fg=self.colors["accent_cyan"],
            bg=self.colors["bg_primary"],
        ).pack(pady=20)
        ...
        for symbol, data in self.portfolio.items():
            ...
            current_price = data.get("current_price", data.get("avg_price", 0))
            value = shares * current_price
            investment_listbox.insert(
                tk.END, f"{symbol}: {shares} shares (${value:,.2f})"
            )
                ...
                if messagebox.askyesno(
                    ...
                    if symbol in self.portfolio:
                        ...
                        self.add_transaction_record(
                            ...
                            credit_amount,
                            f"Removed {shares} shares at ${current_price:,.2f}",
                        )
    ...
    def delete_financial_goal(self):
        ...
        if not selected:
            messagebox.showinfo(
                "Selection Required", "Please select a goal from the list to delete."
            )
            return
        ...
        if selected[0] < header_lines:
            messagebox.showinfo(
                "Invalid Selection",
                "Please select a goal from the list, not the header.",
            )
            return
        ...
        if goal_index < 0 or goal_index >= len(self.financial_goals):
            messagebox.showinfo(
                "Invalid Selection", "Please select a valid goal from the list."
            )
            return
        ...
        messagebox.showinfo(
            "Goal Deleted",
            f"Goal '{goal['name']}' has been deleted and ${goal['current']:.2f} has been returned to your cash balance.",
        )
    ...
    def update_financial_goal(self):
        ...
        if not selected:
            messagebox.showinfo(
                "Selection Required", "Please select a goal from the list to update."
            )
            return
    ...
    def allocate_funds_to_goal(self):
        ...
        for i, goal in enumerate(self.financial_goals):
            ...
            goal_listbox.insert(
                tk.END,
                f"{goal['name']} (${goal['current']:.2f} / ${goal['target']:.2f}, ${remaining:.2f} needed)",
            )
        ...
        def on_allocate():
            ...
            try:
                ...
                # If not enough cash, use credit card for the difference
                if amount > self.cash_balance:
                    diff = amount - self.cash_balance
                    used_cc = self.use_credit_card_for_goal(diff)
                    if used_cc:
                        amount = self.cash_balance
                if amount > self.cash_balance:
                    messagebox.showwarning(
                        "Warning", "Amount exceeds available cash and credit card limit"
                    )
                    return
                self.cash_balance -= amount
                self.financial_goals[idx]["current"] += float(amount_var.get())
                self.save_data()
                self.update_goals_display()
                self.update_dashboard()
                messagebox.showinfo(
                    "Success", f"${amount_var.get()} allocated to '{goal['name']}'"
                )
                dialog.destroy()
            except ValueError:
    ...
    def update_financial_goal(self):
        ...
        for i, goal in enumerate(self.financial_goals):
            goal_listbox.insert(
                tk.END,
                f"{goal['name']} (${goal['current']:,.0f}/${goal['target']:,.0f})",
            )
    ...
    def _create_goal_dialog(self, title, goal_index):
        ...
        meaning_combo = ttk.Combobox(
            input_frame,
            values=["Savings", "Emergency", "Debt", "Investment", "Other"],
            ...
            try:
                ...
                if name and target > 0:
                    goal = {
                        ...
                        "current": current,
                        "meaning": (
                            meaning_var.get().strip() if meaning_var.get() else "Other"
                        ),
                    }
    ...
    def update_portfolio_prices(self):
        ...
        stock_portfolio = {
            symbol: data for symbol, data in self.portfolio.items() if symbol != "CASH"
        }
        ...
        def update_prices():
            try:
                    ...
                    try:
                        ...
                        if not hist.empty:
                            ...
                            failed_symbols.append(
                                f"{symbol}: No data (possibly delisted)"
                            )
                    except Exception as e:
                        failed_symbols.append(f"{symbol}: {str(e)}")
                ...
                self.root.after(
                    0, lambda: messagebox.showinfo("Update Prices", message)
                )
                self.save_data()
    ...
    def update_portfolio_display(self):
        ...
        try:
            ...
            if not self.portfolio or (
                ...
                and "CASH" in self.portfolio
                and self.portfolio["CASH"]["shares"] == 0
            ):
                self.holdings_text.insert(
                    "1.0",
            ...
            for symbol, data in self.portfolio.items():
                ...
                gain_loss_pct = (
                    (gain_loss / invested_value * 100) if invested_value > 0 else 0
                )
            ...
            self.holdings_text.insert(
                "1.0", "Error updating portfolio display. Please try again."
            )
        ...
        try:
            sorted_portfolio = sorted(
                self.portfolio.items(), key=get_position_value, reverse=True
            )
        except Exception:
        ...
        portfolio_color = (
            self.colors["success"] if total_gain_loss >= 0 else self.colors["danger"]
        )
    ...
    def add_financial_goal(self):
        ...
        tk.Checkbutton(
            cash_frame,
            text="Use cash balance for initial contribution",
            variable=use_cash_var,
            font=("Segoe UI", 11),
            fg=self.colors["text_primary"],
            bg=self.colors["bg_primary"],
            selectcolor=self.colors["bg_secondary"],
            command=lambda: current_var.set("0") if not use_cash_var.get() else None,
        ).pack(anchor="w")
        ...
        tk.Label(
            note_frame,
            text="Note: Using cash balance will reduce your available cash.",
    ...
    def update_goals_display(self):
        ...
        if not hasattr(self, "goals_listbox") or self.goals_listbox is None:
            print(
                "Error updating dashboard: 'FinanceDashboard' object has no attribute 'goals_listbox'"
            )
            return
        ...
        if not self.financial_goals:
            self.goals_listbox.insert(
                tk.END, "No financial goals yet. Click 'Add Goal' to get started!"
            )
        ...
        # Add column headers
        self.goals_listbox.insert(
            tk.END, f"{'Goal':<25} {'Target':<15} {'Current':<15} {'Progress':<20}"
        )
        ...
        completed_goals = sum(
            1 for goal in self.financial_goals if goal["current"] >= goal["target"]
        )
        total_target = sum(goal["target"] for goal in self.financial_goals)
        total_current = sum(goal["current"] for goal in self.financial_goals)
        overall_progress = (
        ...
        self.goals_listbox.insert(
            tk.END,
            f"Total Goals: {total_goals} | Completed: {completed_goals} | Overall Progress: {overall_progress:.1f}%",
        )
        self.goals_listbox.insert(
            tk.END,
            f"Total Target: ${total_target:,.0f} | Total Saved: ${total_current:,.0f}",
        )
        ...
        tk.Button(
            ...
            command=self.contribute_to_goal,
            bg=self.colors["success"],
            fg=self.colors["bg_primary"],
            font=("Segoe UI", 10, "bold"),
            padx=15,
    ...
    def edit_goal(self):
        ...
        try:
            ...
            if (
                ...
                or selected_text.startswith("FINANCIAL")
                or selected_text.startswith("Total")
            ):
            ...
            if not goal_to_edit:
                messagebox.showinfo(
                    "Goal Not Found", "Could not find the selected goal"
                )
                return
            ...
            def save_edited_goal():
                ...
                try:
                    ...
                    if name and target > 0:
                        ...
                        goal_to_edit["meaning"] = (
                            meaning_var.get().strip()
                            if meaning_var.get()
                            else goal_to_edit.get("meaning", "Other")
                        )
                    ...
                    messagebox.showerror(
                        "Error", "Please enter valid numbers for amounts"
                    )
    ...
    def delete_goal(self):
        ...
        try:
            ...
            if (
                ...
                messagebox.showinfo(
                    "Select Goal", "Please select a valid goal to delete"
                )
                return
    ...
    def contribute_to_goal(self):
        ...
        try:
            ...
            if (
                ...
                messagebox.showinfo(
                    "Select Goal", "Please select a valid goal to contribute to"
                )
                return
            ...
            tk.Label(
                dialog,
                text=f"Contribute to: {goal_name}",
                font=("Segoe UI", 16, "bold"),
                fg=self.colors["accent_purple"],
                bg=self.colors["bg_primary"],
            ).pack(pady=20)
            ...
            def make_contribution():
                try:
                    ...
                    dialog.destroy()
                    messagebox.showinfo(
                        "Success", f"Contributed ${amount:,.2f} to {goal_name}"
                    )
                ...
                for i, wedge in enumerate(wedges):
                    if wedge.contains_point([event.x, event.y]):
                        ...
                        ax.text(
                            ...
                            bbox=dict(
                                boxstyle="round,pad=0.5", facecolor="black", alpha=0.8
                            ),
                            color="white",
                            fontsize=12,
                            fontweight="bold",
    ...
    def show_line_chart(self):
        ...
        ax.set_ylabel(
            "Cumulative Amount ($)", color="white", fontsize=12, fontweight="bold"
        )
...
def main():
    ...
    root.attributes("-fullscreen", True)  # For other OSes (may be ignored on Windows)
